...
Block::Block(const sf::Texture &texture, float left, float top) : Creature(texture)
{
...
BrickBlock::BrickBlock(const sf::Texture &texture, float left, float top) : Block(texture, left, top) {
    _sprite.setTextureRect(sf::IntRect(144, 112,
                                       texture_size, texture_size));
}

void BrickBlock::update(float &offsetX){
...
LuckyBlock::LuckyBlock(const sf::Texture &texture, float left, float top) : Block(texture, left, top)
{
...
void LuckyBlock::update(float &offsetX){
    animation();
